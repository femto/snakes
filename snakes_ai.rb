  ...
  if foods.empty? #no gems find
    ...
    0.upto(2) do |index|
      ...
      if path && clever && !trap(info, seq, path, enemy_food_type)
        ...
        cost_and_dirs << [path.length, dir,index, food]

      end
    end
    min_cost = cost_and_dirs.min_by {|x|x[0]}
    if min_cost[2] != 0
      puts "choosing index #{min_cost[2]} of cost #{min_cost[0]} for food [#{min_cost[3][0]},#{min_cost[3][1]}]"
    end
...
def game_loop
  ...
  while _info && _info["status"] == "running"
    begin
      sleep 0.15
      _turn, _info = turn
      #p _info
